...
- $P$ la matriz de permutación. Su única tarea en la vida es almacenar las permutaciones (Cambio de filas) que haya.
- $L$ sigue siendo una triangular inferior.
- $U$ sigue siendo la triangular superior.

Partimos que: $Ax = B$, luego vemos que $LUx = B$, sin embargo, la tarea de $P$ es almacenar las permutaciones y aplicarlas a $B$. Luego, entonces el sistema queda de la siguiente manera: $PLUx = PB$
...
??? example "Ejemplo 1"
    ...
    Note que $u_{22} = 0$ entonces hay que hacer un cambio de filas ($F_2$ por $F_3$):
    ...
    - $L$ solo cambió las filas en sus valores almacenados en la triangular inferior.
...
??? example "Ejemplo 2"
    ...
    Dando $y_1 = 1, y_2 = 4, y_3 = -8$. Luego resolvemos $Ux = y$:
